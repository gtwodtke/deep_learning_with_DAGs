...
folder = 'blau_duncan_1967_sens6'  # Define the folder where files will be stored.
...
process(path=path, dataset_name=dataset_name, dag_name=dataset_name + '_DAG', test_size=0.2, cat_var=['U', 'V', 'W', 'X', 'Y'], sens_corr=corr_strengths, seed=None)

train(path=path, dataset_name=dataset_name, model_name =path + 'point',
...
#ATE of U on Y
sim(path=path, dataset_name=dataset_name, model_name =path + 'point', n_mce_samples = 10000, treatment='U', cat_list=list(range(9)), moderator=None, mediator=None, outcome='Y', inv_datafile_name='ATE_UY')
