...
file_names = [f'ATE_XY_final_results_{i}.csv' for i in range(1, 16)]
...
final_results_file_path = os.path.join(path, 'ATE_XY_final_results.csv')
...
outcome_columns = [col for col in final_results_df if col.startswith('E[Y(X=')]
...
percentiles_csv_path = path + 'percentiles_ATE_XY_final_results.csv'
percentiles_df.to_csv(percentiles_csv_path)

percentiles_df = pd.read_csv(path + '/percentiles_ATE_XY_final_results.csv')
...
point_results = pd.read_csv(base_path + '/point_ATE_XY_results.csv')
...
final_results.to_csv(base_path + '/final_ATE_XY_results.csv')
