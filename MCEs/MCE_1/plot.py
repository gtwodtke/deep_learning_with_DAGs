...
# Reading datasets
path = '/Users/changhuizhou/Desktop/Midway_MCE_results/LINEAR/'
...
label_effect = ["ATE (A→Y)", "PSE (A→Y)", "PSE (A→L→Y;A→L→M→Y)", "PSE (A→M→Y)", "ATE (A→M)", "NDE (A→M)", "NIE (A→L→M)"]
...
plt.xlabel("Sample Size")
plt.ylabel("Difference in Square Root of MSE")
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.legend()
plt.tight_layout()
plt.ylim(bottom=0)  # Set the lower limit of y-axis to 0
