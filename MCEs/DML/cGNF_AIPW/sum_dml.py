...
base_path = '/Users/jessezhou/Desktop/Revision/cGNF_DML'
folder = '1k'
...
final_results_df['cGNF_ATE (A->Y) with empirical sampling'] = final_results_df['E_Y_A_1_boot'] - final_results_df['E_Y_A_0_boot']
...
true_values = {
    'cGNF_ATE (A->Y)': 0.3,
    'cGNF_ATE (A->Y) with empirical sampling': 0.3,
    'ATEhat': 0.3,
    'ATE_dml': 0.3
}
...
columns_to_analyze = ['cGNF_ATE (A->Y)', 'cGNF_ATE (A->Y) with empirical sampling', 'ATEhat', 'ATE_dml']
summary_stats = {}
...
summary_stats_df.to_csv(path + '1k_summary_statistics.csv', index=True)
