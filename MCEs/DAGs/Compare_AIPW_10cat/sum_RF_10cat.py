...
base_path = '/Users/jessezhou/Desktop/Revision/RFs_10cat_new'
folder = '64k'
path = os.path.join(base_path, folder, '')

# # List of file names to be appended
# file_names = [f'64k_final_results_{i}.csv' for i in range(1, 6)]
#
...
final_results_file_path = os.path.join(path, '64k_final_results.csv')
...
final_results_df['cGNF_ATE (A->Y)'] = final_results_df['E_Y_A_7_sim'] - final_results_df['E_Y_A_2_sim']
final_results_df['cGNF_ATE (A->Y) with bootstrap'] = final_results_df['E_Y_A_7_boot'] - final_results_df['E_Y_A_2_boot']
...
true_values = {
    'cGNF_ATE (A->Y)': 1.5,
    'cGNF_ATE (A->Y) with bootstrap': 1.5,
    'ATEhat': 1.5
}


# Compute the mean, variance, and MSE for the specified columns
columns_to_analyze = ['cGNF_ATE (A->Y)', 'cGNF_ATE (A->Y) with bootstrap', 'ATEhat']
summary_stats = {}
...
summary_stats_df.to_csv(path + '64k_summary_statistics.csv', index=True)
