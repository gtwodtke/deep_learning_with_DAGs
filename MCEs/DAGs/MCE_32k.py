...
def run_simulation(i, node_id):
    ...
    # Unobserved variable
    U = np.random.normal(0, 1, obs)

    # Baseline Confounders affected by U
    C1 = np.random.normal(0, 1, obs) + 0.3 * U
    C2 = np.random.normal(0, 1, obs) + 0.4 * U
    ...
    # Standard normal Y
    Y = 0.1 * A + 0.2 * C2 + 0.2 * A * C2 + epsilon_Y
